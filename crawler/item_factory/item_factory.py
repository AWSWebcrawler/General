...
def create_items(html: str, url: str) -> dict:
    ...
    items = {
        ...
        "asin": get_asin(tree),
        "url": get_url(url),
        "timestamp": get_timestamp(datetime_now),
        "date": get_date(datetime_now),
        "time": get_time(datetime_now),
    }
...
@myannotation
def get_date(date_time_current):
    """Returns the date part of the given datetime"""
    return date_time_current.strftime("%Y-%m-%d")
...
def get_url(url):
    ...
    if not re.match("^https://www.amazon.de", url):
...
def get_percent_discount(tree):
    ...
    # TODO create two private methods (with underscore "_abc")
...
def get_amazon_choice(tree):
    ...
    if div_tag is None:
        ...
        return (span_tags is not None) and (len(span_tags) > 1)
