...
class TestItemFactory(unittest.TestCase):
    ...
    def test_create_item(self):
        ...
        expected = 'https://www.amazon.de/der-neue-echo-dot-4-generation-smarter-lautsprecher-mit-alexa-anthrazit/' \
                   'dp/B084DWG2VQ'
        self.assertEqual(expected, product["url"], "The created item url does not match the expected output.")

        product = item_factory.create_item(self.test_html['test_html_2'], self.urls['url2'])
        expected = 'https://www.amazon.de/Xbox-Wireless-Controller-Electric-Volt/dp/B091CK241X'
        self.assertEqual(expected, product["url"], "The created item url does not match the expected output.")

        product = item_factory.create_item(self.test_html['test_html_3'], self.urls['url3'])
        expected = 'https://www.amazon.de/FLAMMBURO-Paraffinbasis-Grillanz%C3%BCnder-Kaminanz%C3%BCnder-Paraffinw' \
                   '%C3%BCrfel/dp/B08YCWDLTQ'
        self.assertEqual(expected, product["url"], "The created item url does not match the expected output.")

        product = item_factory.create_item(self.test_html['test_html_4'], self.urls['url4'])
        expected = 'https://www.amazon.de/CYBERPUNK-2077-DAY-Standard-Xbox/dp/B07SF1LZ9Q'
        self.assertEqual(expected, product["url"], "The created item url does not match the expected output.")
