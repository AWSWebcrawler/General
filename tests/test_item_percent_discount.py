"""Class to test the item percent_discount."""
import unittest
...
class TestItemFactory(unittest.TestCase):
    """Test Class for item percent_discount"""
    ...
    def test_create_item(self):
        ...
        expected = 51.0
        self.assertEqual(expected, product["percent_discount"],
                         "The created item percent_discount does not match the expected output.")

        product = item_factory.create_item(self.test_html['test_html_2'], self.urls['url2'])
        self.assertIsNone(product["percent_discount"],
                          "The created item percent_discount does not match the expected output.")
