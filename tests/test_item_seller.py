...
class TestItemFactory(unittest.TestCase):
    ...
    def test_create_item(self):
        ...
        expected = 'Amazon'
        self.assertEqual(expected, product["seller"], "The created item seller does not match the expected output.")

        product = item_factory.create_item(self.test_html_2, self.url2)
        self.assertIsNone(product["seller"], "The created item seller does not match the expected output.")
