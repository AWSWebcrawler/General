...
class TestItemFactory(unittest.TestCase):
    """Test Class for item discount_in_euros"""
    ...
    def test_create_item(self):
        ...
        expected = 30.81
        self.assertEqual(expected, product["discount_in_euros"],
                         "The created item discount_in_euros does not match the expected output.")

        product = item_factory.create_item(self.test_html_2, self.url2)
        self.assertIsNone(product["discount_in_euros"],
                          "The created item discount_in_euros does not match the expected output.")
