...
class TestItemFactory(unittest.TestCase):
    ...
    def test_create_item(self):
        ...
        expected = 59.99
        self.assertEqual(expected, product["price_regular"], "The created item price_regular does not match the expected output.")

        product = item_factory.create_item(self.test_html_2, self.url2)
        self.assertIsNone(product["price_regular"], "The created item price_regular does not match the expected output.")
